...
nuts <- read_excel("nuts.xlsx") %>%
  mutate(snack_peanuts = parse_number(snack_peanuts)) %>%
  mutate(peanut_butter = parse_number(peanut_butter)) %>%
  mutate(peanut_candy = parse_number(peanut_candy)) %>%
...
indiv_availability[is.na(indiv_availability)] <- 0

grouped <- indiv_availability %>%
  mutate(perishable = retail_chicken + milk_cream + butter + total_cheese + fresh_fruit + fresh_veg + indiv_fish_shell + total_wheat_flour + oat + snack_peanuts + peanut_butter + peanut_candy + total_peanuts + indiv_meat) %>%
  mutate(imperishable = total_frozen_dairy + total_canned_dairy + total_dry + indiv_fresh_frozen_fish + indiv_canned_fish + indiv_cured + processed_fruit + processed_veg + total_sweeteners)
```
